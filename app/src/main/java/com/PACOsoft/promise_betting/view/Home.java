...
//        //약속 정보 DB에 접근해서 내 약속과 일치하는 약속들만 어뎁터 연결해서 리사이클러뷰에 띄워주고 newPromises에 저장
    ...
    public void lead_friends(friendsCallback callback) {
        ...
        databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    if (snapshot.getKey().toString().equals("friendsUID")) {
                        String[] friends = snapshot.getValue().toString().split(" ");//위에서 필터링한 객체의 FriendsId를 공백을 기준으로 스플릿 해서 배열에 저장
                        callback.onCallback(friends);
                    } else if (snapshot.getKey().toString().equals("account")) {
                        coin = Integer.valueOf(snapshot.getValue().toString());//내 객체에서 account값 가져옴
                        TextView text = (TextView) findViewById(R.id.tv_point);//TextView 참조 객체 선언
                        text.setText(String.valueOf(coin));//위에서 선언한 참조 객체에 값 넘겨줌
                    }
                }
...
//
    ...
    public void view_friends() {
        ...
        lead_friends(new friendsCallback() {

            @Override
            public void onCallback(String[] temp) {
